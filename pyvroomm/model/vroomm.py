...
class vroomm:
    def __init__(self,modelname,wdir="./"):
        self.zemax_file = ""
        self.wdir = Path(wdir)
        self.hdf = join(self.wdir,".hdf")
        self.simulationdir = join(self.wdir,".simulation")
        self.orderdir = join(self.wdir,".hdf")
        self.modelname = modelname
        self.cfgfile = join(self.wdir,"%s.txt"%self.modelname)
        self.simulationname = ""
        self.order_list_f1 = join(self.orderdir,"%s.fiber1.ordes"%self.modelname)
        #to be configure
        self.minorder = 65
        self.maxorder = 155
        self.grating_idx = 5 
    ...
    def get_order_data(self,order,fiber=1):
        ...
        return data
    def _load(self):
        if all([isdir(self.wdir),isdir(self.orderdir),isfile(self.cfgfile)]):
            print("Model Found")
            print("Loading model")
